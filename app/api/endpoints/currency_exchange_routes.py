...
async def create_currency_exchange_route(
    ...
    exchange_id = await CurrencyExchangeController.create_currency_exchange(exchange_schema, user)
...
async def read_currency_exchange_route(
    exchange_id: str = Path(..., description="The ID of the currency exchange to retrieve"),
    user=Depends(has_role(R.USER)),
):
    exchange = await CurrencyExchangeController.get_currency_exchange(exchange_id, user.id)
    return ResponseSchema(
        data={"currency_exchange": exchange}, message="Currency exchange retrieved successfully"
    )

@router.get("", response_model=ResponseSchema)
async def read_all_currency_exchanges_route(user=Depends(has_role(R.USER))):
...
async def update_currency_exchange_route(
    ...
    exchange_id: str = Path(..., description="The ID of the currency exchange to update"),
    user=Depends(has_role(R.USER)),
):
...
async def delete_currency_exchange_route(
    exchange_id: str = Path(..., description="The ID of the currency exchange to delete"),
    user=Depends(has_role(R.USER)),
):
    success = await CurrencyExchangeController.delete_currency_exchange(exchange_id, user.id)
